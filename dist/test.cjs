...
class UpdateDecoderV1 extends DSDecoderV1 {
   ...
   */
  readBuf () {
    return buf__namespace.copyUint8Array(decoding__namespace.readVarUint8Array(this.restDecoder))
  }
 ...
 */
const isVisible = (item, snapshot) => snapshot === undefined ? !item.deleted : (snapshot.sv.has(item.id.client) && /** @type {number} */
    (snapshot.sv.get(item.id.client)) > item.id.clock && !isDeleted(snapshot.ds, item.id));
    ...
    _renderSnapshot(snapshot$1, prevSnapshot, pluginState) {
        ...
        if (snapshot$1 instanceof Uint8Array || prevSnapshot instanceof Uint8Array) {
            ...
            if (historyType._item === null) {
                ...
                const itemIndex = findIndexSS(historyStructs, historyType._item.id.clock);
    ...
    _typeChanged(events, transaction) {
        ...
        this.mux(() => {
            ...
            tr = tr.setMeta(ySyncPluginKey, {
                isChangeOrigin: true, isUndoRedoOperation: transaction.origin instanceof UndoManager
            });
...
const createNodeFromYElement = (el, schema, meta, snapshot, prevSnapshot, computeYChange) => {
    ...
    if (snapshot === undefined || prevSnapshot === undefined) {
        ...
        if (existedInPrev) {
            typeListToArraySnapshot(el, prevSnapshot).forEach(createChildren);
        } else {
...
const equalYTextPText = (ytext, ptexts) => {
    ...
    return delta.length === ptexts.length && delta.every(/** @type {(d:any,i:number) => boolean} */(d, i) => d.insert === /** @type {any} */ (ptexts[i]).text && object__namespace.keys(d.attributes || {}).length === ptexts[i].marks.length && object__namespace.every(d.attributes, (attr, /** @type {string} */ yattrname) => {
...
const schema = new PModel.Schema({
  nodes: basicSchema__namespace.nodes,
  marks: Object.assign({}, basicSchema__namespace.marks, {
    comment: {
      attrs: {
        id: { default: null }
      },
 ...
 * @param {t.TestCase} tc
 */
const testOverlappingMarks = (_tc) => {
  const view = new prosemirrorView.EditorView(null, {
    state: prosemirrorState.EditorState.create({
      schema,
      plugins: []
  ...
  t__namespace.compare(backandforth.content[0].content, JSON.parse(expected));
};
...
const testReplaceBoldWithCode = (_tc) => {
  const ydoc = new Doc();
  const yXmlFragment = ydoc.get('prosemirror', YXmlFragment);
...
const testVersioningWithGarbageCollection = async (_tc) => {
  const ydoc = new Doc();
  const yxml = ydoc.get('prosemirror', YXmlFragment);
  const permanentUserData = new PermanentUserData(ydoc);
  permanentUserData.setUserMapping(ydoc, ydoc.clientID, 'me');
...
const pmChanges = [
  ...
  (_y, gen, p) => { // format text
    const insertPos = prng__namespace.int32(gen, 0, p.state.doc.content.size);
    const formatLen = math__namespace.min(
      prng__namespace.int32(gen, 0, p.state.doc.content.size - insertPos),
      2
    );
    const mark = prng__namespace.oneOf(gen, marksChoices.filter(choice => choice.length > 0))[0];
 ...
 * @param {t.TestCase} _tc
 */
const testRestoreSelectionForDeletedInlineNode = (_tc) => {
  const ydoc = new Doc();
  const schemaWithInlineAtom = new PModel.Schema({
    nodes: Object.assign({}, basicSchema__namespace.nodes, {
      inlineatom: {
        inline: true,
        group: 'inline',
        atom: true,
    ...
    marks: basicSchema__namespace.marks
  });
  ...
  t__namespace.assert(!(sel instanceof NodeSelection), 'selection should not be a NodeSelection');
  t__namespace.assert(sel instanceof prosemirrorState.TextSelection, 'selection should be a TextSelection');
  t__namespace.assert(sel.anchor >= 0 && sel.anchor <= view.state.doc.content.size, 'selection anchor within bounds');
};

const testRestoreSelectionForDeletedBlockNode = async (_tc) => {
  const ydoc = new Doc();
  const view = createNewComplexProsemirrorView(ydoc);
  ...
  view.dispatch(
    view.state.tr.insert(
      0,
      [
        schema$1.node('paragraph', undefined, schema$1.text('before')),
        schema$1.node('custom'),
        schema$1.node('paragraph', undefined, schema$1.text('after'))
      ]
    )
  );
  ...
  const sel = view.state.selection;
  t__namespace.assert(sel instanceof NodeSelection, 'selection should be a NodeSelection for block node');
};
...
var prosemirror = /*#__PURE__*/Object.freeze({
  ...
  testOverlappingMarks: testOverlappingMarks,
  ...
  testReplaceBoldWithCode: testReplaceBoldWithCode,
