    ...
    _renderSnapshot(snapshot, prevSnapshot, pluginState) {
        ...
        if (snapshot instanceof Uint8Array || prevSnapshot instanceof Uint8Array) {
            ...
            if (historyType._item === null) {
                ...
                const itemIndex = Y__namespace.findIndexSS(historyStructs, historyType._item.id.clock);
        ...
        this.mux(() => {
            historyDoc.transact((transaction) => {
                ...
                const fragmentContent = Y__namespace.typeListToArraySnapshot(historyType, new Y__namespace.Snapshot(prevSnapshot.ds, snapshot.sv)).flatMap((t) => {
    ...
    _typeChanged(events, transaction) {
        ...
        this.mux(() => {
            ...
            tr = tr.setMeta(ySyncPluginKey, {
                isChangeOrigin: true, isUndoRedoOperation: transaction.origin instanceof Y__namespace.UndoManager
            });
...
const createNodeFromYElement = (el, schema, meta, snapshot, prevSnapshot, computeYChange) => {
    ...
    if (snapshot === undefined || prevSnapshot === undefined) {
        ...
        if (existedInPrev) {
            Y__namespace.typeListToArraySnapshot(el, prevSnapshot).forEach(createChildren);
        } else {
